...
namespace db_api {
  ...
  bool DBConnectionPool::m_init = false;
  DBConnectionPool* DBConnectionPool::m_instance = 0;
  ...
  /*
   *   Paramater: delta. If negative the pool will shrink by delta, if positive the pool
   *   will grow by delta.
  ...
  /*
   *  Reset the pool to a new size. Same as if the pool was destroyed and created
   *  the with new size.
   */
  ...
  /*
   *    Lookup for a previously saved exclusive connection, passing the connection as the parameter.
   *    If it is found in the pool, the function returns the connection, otherwise returns null  
   */
